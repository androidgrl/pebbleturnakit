...
static BitmapLayer *b_background_layer;
static GBitmap *b_background_bitmap;
...
void init() {
  ...
  b_background_bitmap = gbitmap_create_with_resource(RESOURCE_ID_BLACK);
  b_background_layer = bitmap_layer_create(GRect(0, 144, 144, 24));
  bitmap_layer_set_bitmap(b_background_layer, b_background_bitmap);
  layer_add_child(window_get_root_layer(window), bitmap_layer_get_layer(b_background_layer));
...
void deinit() {
  gbitmap_destroy(s_background_bitmap);
  bitmap_layer_destroy(s_background_layer);
  gbitmap_destroy(b_background_bitmap);
  bitmap_layer_destroy(b_background_layer);
  text_layer_destroy(text_layer);
